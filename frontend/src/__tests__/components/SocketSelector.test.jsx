...
describe('SocketSelector', () => {
  ...
  it('selected lens has border-accent-border class', async () => {
    listLenses.mockResolvedValue([
      { id: 'action_items', name: 'Action Items' },
      { id: 'summary', name: 'Summary' },
    ]);
    ...
    await waitFor(() => {
      expect(selectedButton.closest('button').className).toContain('border-accent-border');
    });

    const unselectedButton = screen.getByText('Summary');
    expect(unselectedButton.closest('button').className).not.toContain('border-accent-border');
  });

  it('selected socket has border-accent-border class', async () => {
    listLenses.mockResolvedValue([]);
    listSockets.mockResolvedValue([
