...
describe('SocketSelector', () => {
  ...
  it('selected lens has border-neutral-900 class', async () => {
    listLenses.mockResolvedValue([
      { id: 'action_items', name: 'Action Items' },
      { id: 'summary', name: 'Summary' },
    ]);
    ...
    await waitFor(() => {
      expect(selectedButton.closest('button').className).toContain('border-neutral-900');
    });

    const unselectedButton = screen.getByText('Summary');
    expect(unselectedButton.closest('button').className).not.toContain('border-neutral-900');
  });

  it('selected socket has border-neutral-900 class', async () => {
    listLenses.mockResolvedValue([]);
    listSockets.mockResolvedValue([
